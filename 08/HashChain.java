public class HashChain {
    ...
    public HashChain (int size) {
        ...
        for (int i = 0; i < size; i++) {
            this.table[i] = new LinkBucket(null, "dummy");
        }
    }
    ...
    public void put (String key, Object data) {
        ...
        LinkBucket linkBucket = this.table[index];
    ...
    public Object get (String key) {
        int index = HashUtils.hash(this.table.length, key);
        LinkBucket dummy = this.table[index];
    ...
    public Object remove (String key) {
        int index = HashUtils.hash(this.table.length, key);
        LinkBucket preLinkBucket = this.table[index];
        LinkBucket linkBucket = preLinkBucket.getNext();
        while (linkBucket != null) {
            if (key.equals(linkBucket.getKey())) {
                preLinkBucket.setNext(linkBucket.getNext());
                return linkBucket.getData();
            }
